...
`;
  ...
  try {
    ...
    // Never log the raw error: axios attaches the request headers to it,
    // including the OpenAI API key.
    const detail = error.response?.data?.error
    console.error(`OpenAI request failed: ${error.response?.status} ${detail?.code ?? ''} ${detail?.message ?? error.message}`)
    return res.status(502).json({ error: 'DietGPT no fit answer right now. Abeg try again later.' })
  }
}
